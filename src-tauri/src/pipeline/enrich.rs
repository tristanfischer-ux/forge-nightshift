...
pub async fn run(app: &tauri::AppHandle, job_id: &str, config: &Value) -> Result<Value> {
    ...
    // Load only companies that have a website
    let companies = {
        let db: tauri::State<'_, Database> = app.state();
        db.get_enrichable_companies(500)?
    };
...

    {
        ...
        let _ = db.log_activity(
            ...
            "info",
            &format!(
                "Starting enrichment for {} companies (concurrency={}) using model: {} (auto-approve: relevance>={}, quality>={})",
                total, concurrency, enrich_model, relevance_threshold, quality_threshold
            ),
        );
    }

    stream::iter(companies.into_iter().enumerate())
        .map(|(idx, company)| {
            let app = app.clone();
...
Return ONLY valid JSON. /no_think"#,
    ...
    Ok(json!({
        ...
        "errors": final_errors,
        "no_website_filtered": no_website_count,
    }))
}
